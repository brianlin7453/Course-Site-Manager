...
public abstract class AppTemplate extends Application {
	...
	AppESDialogSingleton ESDialog = AppESDialogSingleton.getSingleton();
	ESDialog.init(primaryStage);

	try {
	    // LOAD APP PROPERTIES, BOTH THE BASIC UI STUFF FOR THE FRAMEWORK
	    // AND THE CUSTOM UI STUFF FOR THE WORKSPACE
            AppESDialogSingleton ES = AppESDialogSingleton.getSingleton();
            ES.show("English or Spanish","English or Spanish?");
            System.out.println(ES.getSelection());
            boolean success;
            if(ES.getSelection().equals("English"))
                success = loadProperties(APP_PROPERTIES_FILE_NAME);
	    else
                success = loadProperties(SAPP_PROPERTIES_FILE_NAME);
	    if (success) {
