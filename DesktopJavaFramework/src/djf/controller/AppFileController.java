...
public class AppFileController {
    ...
    public void handleExportRequest() {
        export(new File(PATH_PUBLIC_HTML));
    }
    ...
    public void export(File file) {
        ...
        setSource(file);
        try {
            DirectoryChooser exportDest = new DirectoryChooser();
            dest = exportDest.showDialog(app.getGUI().getWindow());
            setDest(dest);
            app.getFileComponent().exportData(app.getDataComponent(),PATH_WORK);
            String getJSONCourse = dest + "/js/ProjectsData.json";
            String getJSONTA = dest + "/js/OfficeHoursGridData.json";
            String getJSONRec = dest + "/js/RecitationsData.json";
            String getJSONSch = dest + "/js/ScheduleData.json";
            String getJSONPro = dest + "/js/TeamsAndStudents.json";
            String getJSONC = dest + "/js/CourseData.json";
            //System.out.println(getJSON);
            File data1 = new File(getJSONCourse);
            File data2 = new File(getJSONTA);
            File data3 = new File(getJSONRec);
            File data4 = new File(getJSONSch);
            File data5 = new File(getJSONPro);
            File data6 = new File(getJSONC);
            data1.delete();
            data2.delete();
            data3.delete();
            data4.delete();
            data5.delete();
            data6.delete();
            app.getFileComponent().saveTeamAndStudentData(app.getDataComponent(), data1.getPath());
            app.getFileComponent().saveTAData(app.getDataComponent(), data2.getPath());
            app.getFileComponent().saveRecData(app.getDataComponent(), data3.getPath());
            app.getFileComponent().saveSchData(app.getDataComponent(), data4.getPath());
            app.getFileComponent().saveProjectData(app.getDataComponent(), data5.getPath());
            app.getFileComponent().saveCourseData(app.getDataComponent(), data6.getPath());
        }
        catch(IOException e){
